...
#source("D:/Disco C/Repositorios Git/Labor-Market-and-Compositional-Analysis/Compositional Data and Labor MArket/R/Programs/functions_Pandemic_Priors.R")
source('C:/Users/ojaulime/OneDrive - Banco de la República/Documents/Research/Labor MArket and Compositional Data/Labor-Market-and-Compositional-Analysis/Compositional Data and Labor MArket/R/Programs/functions_Pandemic_Priors.R')
...
covid_periods <- 12L         # March 2020 onward; set to 0 for no pandemic dummies
...
# ------------------------------------------------------------
# 8. Posterior draws under Cholesky identification
# ------------------------------------------------------------
...
message("Drawing posterior coefficients under pandemic priors...")
...
# Cascaldi-Garcia's do_irfs() returns [shock, horizon, response].
# Convert to your original bsvars convention: [response, shock, horizon, draw].
...
irfs <- array(
  ...
  dimnames = list(
    response = var_names,
    shock = var_names,          # Cholesky shock names follow the ordering of Y
    horizon = 0:nSteps,
    draw = seq_len(rps)
  )
)
...
pb <- txtProgressBar(min = 1, max = rps, initial = 1, style = 3)
message("Computing Cholesky IRFs...")
for (iii in seq_len(rps)) {
  setTxtProgressBar(pb, iii)
  irf_i <- do_irfs(
...
irf_summary <- irf_df |>
  ...
  summarise(
    ...
    lower90 = quantile(irf, probs = 0.05, na.rm = TRUE),
    upper90 = quantile(irf, probs = 0.95, na.rm = TRUE),
    .groups = "drop"
  )
