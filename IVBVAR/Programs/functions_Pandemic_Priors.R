...
draw_coef_pandemic_priors_stab <- function(SSE_post, v1, chol_ixx, A_post, n, nAR, covid_periods) {
  ...
  while (control2 == 0) {
    ...
    if (eigAR_a < 1.01) {
      control2 <- 1
      return(list("A_companion_dr" = A_companion_dr, "A0hat" = A0hat, "nbeta_dr" = nbeta_dr))
    } else {
      next
    }
  } 
...
get_IMP_IV_draw <- function(nbeta_draw, A0hat_chol, Y, X, Z_instrument, policy_idx) {
  # 1. Calcular los residuos de forma reducida para el draw actual
  # nbeta_draw ya incluye el efecto de los dummies, así que los residuos quedan "limpios"
  resid_draw <- Y - X %*% nbeta_draw
  
  # 2. Filtrar solo los periodos donde el instrumento (SHOCKOISL) tiene datos
  valid_idx <- which(!is.na(Z_instrument))
  Z_valid <- Z_instrument[valid_idx]
  v_valid <- resid_draw[valid_idx, , drop = FALSE]
  
  # 3. Regresión IV (Proyección sobre el instrumento)
  # crossprod(v_valid, Z_valid) equivale a t(v_valid) %*% Z_valid
  h1_iv <- crossprod(v_valid, Z_valid) / sum(Z_valid^2)
  
  # 4. Normalización
  # Forzamos a que un choque positivo aumente la variable de política en 1 unidad
  h1_norm <- (h1_iv / h1_iv[policy_idx])*0.25
  
  # 5. Construir la matriz de impacto compatible con do_irfs
  # Usamos la Cholesky original como "molde" para que el código iterativo no colapse
  IMP_IV <- A0hat_chol 
  
  # Reemplazamos SOLAMENTE la columna que corresponde a la variable de política
  # con nuestro choque identificado por variables instrumentales
  IMP_IV[, policy_idx] <- h1_norm
